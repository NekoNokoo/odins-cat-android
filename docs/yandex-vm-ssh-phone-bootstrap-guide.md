# Odin One: SSH для Yandex VM и телефона
...
3. Получить приватный ключ в том текстовом формате, который потом можно вставить в телефон для Odin One.
...
Для Odin One сейчас самый безопасный сценарий такой:
...
Для Odin One в MVP лучше использовать обычный `SSH key` плюс локального пользователя через `cloud-init`.
...
Если цель: потом зайти на сервер с телефона и развернуть Odin One, то лучше сразу задать локального пользователя через `Metadata -> user-data`.
...
## Как проверить белый IP в клиенте Odin One

После того как у Yandex VM появился публичный IPv4, его можно сразу проверить прямо в Android-клиенте Odin One.
